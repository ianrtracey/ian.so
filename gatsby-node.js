...
const toFilePath = slug => `${slug.slice(1, -1)}.jpg`;
...
const createBlogPost = (node, createPage) => {
  ...
  const headImagePath = toFilePath(node.fields.slug);
  console.log(`headImagePath: ${headImagePath}`);
  createPage({
    path: node.fields.slug,
    component: blogTemplate,
    context: {
      slug: node.fields.slug,
      headerImagePath: headImagePath
